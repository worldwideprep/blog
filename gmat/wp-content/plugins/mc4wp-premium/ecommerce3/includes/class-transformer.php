...
class MC4WP_Ecommerce_Object_Transformer {
	...
	public function order(WC_Order $order) {
		...
		foreach ($items as $item_id => $item) {
			...
			if (!empty($variation_id) && wc_get_product($variation_id) ) {
	...
	public function product(WC_Product $product) {
		...
		if ($product instanceof WC_Product_Variable && ! empty($children) ) {
		...
		// use main product if we have no variants by now.
		if( empty( $variants ) ) {
