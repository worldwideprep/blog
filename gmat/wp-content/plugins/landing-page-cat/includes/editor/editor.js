...
jQuery(document).ready(function($){
	...
	//ADD CODEMIRROR CSS FORMAT HIGHLIGHTING
	var myCodeMirror = CodeMirror.fromTextArea( document.getElementById('fca-pc-codemirror') )

	//DEPLOY MODE TOGGLE
	var LPCsettingsFirstRun = true
	$('#fca_lpc_deploy_mode_input').change(function(){
	...
	$('#fca_lpc_cta_input').change(function(e){
	...
	$('.fca-lpc-show_name').change(function(){
		...
		if ( $('#fca_lpc_cta_input').val() === 'none' || $('#fca_lpc_cta_input').val() === 'button' ) {
	...
	$('.fca_lpc-success_redirect').change(function(){
	...
	$('.fca_lpc-success_mode').change(function(){
		if ( $(this).val() === 'url' ) {
			$('.fca-lpc-success_url').parent().show()
			$('#fca_lpc_page_list_input').parent().hide()
		} else {
			$('.fca-lpc-success_url').parent().hide()
			$('#fca_lpc_page_list_input').parent().show()
		}
	}).change()
	
	//BUTTON REDIRECT MODE / PAGE TOGGLE
	$('.fca_lpc-button_mode').change(function(){
		if ( $(this).val() === 'url' ) {
			$('.fca-lpc-button_url').show()
			$('#fca_lpc_button_page_list_input').parent().hide()
		} else {
			$('.fca-lpc-button_url').hide()
			$('#fca_lpc_button_page_list_input').parent().show()
		}
	}).change()	
	
	//PARTICLE ON/OFF TOGGLE
	$('.fca-lpc-particles_enabled').change(function(){
		$('.fca-lpc-particles-setting').hide()
		if ( $(this).attr('checked') ) {
			$('.fca-lpc-particles-setting').show()
		}
	}).change()
	...
	$('.nav-tab-wrapper a').click(function(e){
		...
		if ( $('#fca_lpc_cta_input').val() === 'button'  ) {
			$('#fca_lpc_landing_page_settings_meta_box, #fca_lpc_landing_page_settings_text_meta_box').hide()
		}
		if ( $('#fca_lpc_cta_input').val() === 'none' ) {
			$('#fca_lpc_landing_page_settings_meta_box, #fca_lpc_landing_page_settings_text_meta_box, #fca_lpc_google_analytics_meta_box').hide()
		}
	}).first().click()
	...
	$('#fca_lpc_welcome_redirect_mode').change(function(){
		if ( $(this).val() === 'all' ) {
	...
	$('#fca_lpc_add_welcome_rule').click(function(){
		if ( $('#fca_lpc_welcome_redirect_mode').val() === 'all' ) {
