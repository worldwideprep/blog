...
function fca_lpc_add_optin_ajax() {
	...
	if ( $nonceVerified && !empty( $email ) && !empty( $provider ) ) {
		...
		switch ( $provider ) {
			...
			default:
				$success = fca_lpc_localwp_subscribe( $post_id, $email, $name );
				break;
		}
...
//SIDEBAR OPTIN ENDPOINT
function fca_lpc_sidebar_submit() {
	
	$nonce = sanitize_text_field( $_REQUEST['nonce'] );
	$nonceVerified = wp_verify_nonce( $nonce, 'fca_lpc_admin_nonce') == 1;
	$email = urlencode ( sanitize_email ( $_REQUEST['email'] ) );
	
	if ( $nonceVerified && !empty ( $email ) ) {
		
		$url =  "https://api.fatcatapps.com/api/activate.php?email=$email&product=landingpagecatsidebar";
...
add_action( 'wp_ajax_fca_lpc_sidebar_submit', 'fca_lpc_sidebar_submit' );